...
import sys
import os
sys.path.append('MVSEP_MDX23_Colab_v2/')
...
def predict_with_model(options):
    ...
    for i, input_audio in enumerate(options['input_audio']):
        ...
        for instrum in model.instruments:
            if instrum == "vocals":
                output_name = os.path.splitext(os.path.basename(input_audio))[0] + '.wav'
            else:
...
def uvr_start(uuid, cpu, overlap_demucs, overlap_MDX, overlap_MDXv3, weight_MDXv3, weight_VOCFT, weight_HQ3, single_onnx, chunk_size, large_gpu, bigshifts, vocals_only, output_format):
    ...
    options = {}
    options['input_audio'] = ["D://WorkSpace/untitled/repository/audio/" + uuid + "/" + uuid + ".wav"]
    options['output_folder'] = "D://WorkSpace/untitled/repository/audio/" + uuid + "/"
    options['cpu'] = cpu
    options['overlap_demucs'] = overlap_demucs
    options['overlap_MDX'] = overlap_MDX
    options['overlap_MDXv3'] = overlap_MDXv3
    options['weight_MDXv3'] = weight_MDXv3
    options['weight_VOCFT'] = weight_VOCFT
    options['weight_HQ3'] = weight_HQ3
    options['single_onnx'] = single_onnx
    options['chunk_size'] = chunk_size
    options['large_gpu'] = large_gpu
    options['bigshifts'] = bigshifts
    options['vocals_only'] = vocals_only
